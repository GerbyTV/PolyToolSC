...
class PricesApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Installeur de Prix version EN - Star Citizen")
        ...
        self.global_ini_url = "https://raw.githubusercontent.com/GerbyTV/PolyToolSC/main/global_en.ini"
        ...
        self.delete_button = ttk.Button(button_frame, text="Supprimer les prix", command=self.delete_translation, state="disabled", style="Delete.TButton")
    ...
    def update_global_ini(self):
        ...
        for branch in branches:
            ...
            if not os.path.exists(french_path):
                continue  # Ignore cette branche si l'installation initiale n'existe pas
    ...
    def create_directories(self, base_path):
        """Crée les répertoires nécessaires pour la traduction."""
        french_path = os.path.join(base_path, "data", "Localization", "french_(france)")
    ...
    def download_file(self, url, destination):
        """
        Télécharge un fichier depuis l'URL et l'enregistre au chemin spécifié.
        """
        try:
            url_with_timestamp = f"{url}?t={int(time.time())}"
            print(f"Téléchargement depuis : {url_with_timestamp}")
            response = requests.get(url_with_timestamp, timeout=10)  # Timeout pour éviter les blocages
            response.raise_for_status()
    ...
    def calculate_online_hash(self, url):
        """
        Calcule le hash SHA256 d'un fichier en ligne, en évitant le cache.
        """
        sha256 = hashlib.sha256()
        try:
            url_with_timestamp = f"{url}?t={int(time.time())}"  # Ajout d'un paramètre dynamique
    ...
    def check_installation_status(self):
        """Vérifie l'état de l'installation et active/désactive les boutons en conséquence."""
        base_path = self.folder_path.get()
        if base_path:
            french_path = os.path.join(base_path, "data", "Localization", "french_(france)")
            global_ini_path = os.path.join(french_path, "global.ini")
    ...
    def check_version_by_hash(self):
        """
        Compare le hash du fichier en ligne et du fichier local pour vérifier les mises à jour.
        """
        base_path = self.folder_path.get()
        if not base_path:
    ...
    def adjust_combobox_width(self, combobox, values):
        """Ajuste la largeur de la Combobox en fonction de la longueur du texte le plus long."""
        if values:
            max_length = max(len(value) for value in values)
            combobox.configure(width=max_length + 5)

    def show_success_popup(self, message):
        """Affiche une pop-up de succès stylisée."""
        top = Tk()
        top.title("Succès")
