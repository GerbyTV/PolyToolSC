...
def main_menu():
    ...
    ttk.Button(main_root, text="Traduction FR", command=lambda: launch_FR(main_root), width=30).pack(pady=5)
    ttk.Button(main_root, text="Traduction FR/EN", command=lambda: launch_traduction(main_root), width=30).pack(pady=5)
    ttk.Button(main_root, text="Affichage de prix - Version EN", command=lambda: launch_prices(main_root), width=30).pack(pady=5)
...
def launch_traduction(root):
    ...
    traduction_root.title("Traduction - Installateur de Traduction FR/EN")
...
def launch_prices(root):
    root.destroy()
    try:
        from app_prices import PricesApp
    except ImportError:
        messagebox.showerror("Erreur", "Impossible de charger l'outil d'affichage de prix.")
        return_to_main(root)
        return

    prices_root = tk.Tk()
    prices_root.title("Affichage de prix - Version anglaise")
    prices_root.geometry("750x600")
    app = PricesApp(prices_root)
    ttk.Button(prices_root, text="Retour au menu", command=lambda: return_to_main(prices_root)).pack(pady=10)
    prices_root.mainloop()

def launch_FR(root):
    root.destroy()
    try:
        from app_fr import FRApp
    except ImportError:
        messagebox.showerror("Erreur", "Impossible de charger l'outil de traduction FR.")
        return_to_main(root)
        return

    FR_root = tk.Tk()
    FR_root.title("Traduction - Installateur de Traduction FR")
    FR_root.geometry("750x600")
    app = FRApp(FR_root)
    ttk.Button(FR_root, text="Retour au menu", command=lambda: return_to_main(FR_root)).pack(pady=10)
    FR_root.mainloop()
